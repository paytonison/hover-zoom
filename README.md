...
- `src/Image Popout (Safari)-1.6.user.js`: the userscript to install.
...
3. Create a new userscript from
   `src/Image Popout (Safari)-1.6.user.js`.
...
Edit `src/Image Popout (Safari)-1.6.user.js` directly, reload the userscript in
...
```sh
node --check 'src/Image Popout (Safari)-1.6.user.js'
```
